...
class ContinuousActivityTests(TestCase):
    ...
    def test_valid_upload(self):
        response = self.client.put(
            reverse(
                "continuousactivity-detail", kwargs={"pk": self.continuousactivity_1.id}
            ),
            data=json.dumps(self.valid_payload),
            content_type="application/json",
        )
    ...
    def test_retrieve_all(self):
        ...
        serializer = ContinuousActivitySerializer(
    ...
    def test_invalid_get_element(self):
        response = self.client.get(
            reverse("continuousactivity-detail", kwargs={"pk": 666})
        )
