...
class StateTests(TestCase):
    def setUp(self):
        ...
        self.state_1 = State.objects.create(
            latitude=random.uniform(0.00, 100.5), longitude=random.uniform(0.00, 100.5)
        )
        self.state_2 = State.objects.create(
            latitude=random.uniform(0.00, 100.5), longitude=random.uniform(0.00, 100.5)
        )
        self.state_3 = State.objects.create(
            latitude=random.uniform(0.00, 100.5), longitude=random.uniform(0.00, 100.5)
        )

        self.valid_payload = {
            "latitude": random.uniform(0.00, 100.5),
            "longitude": random.uniform(0.00, 100.5),
        }
    ...
    def test_valid_upload(self):
        response = self.client.put(
            reverse("state-detail", kwargs={"pk": self.state_1.id}),
            data=json.dumps(self.valid_payload),
            content_type="application/json",
        )
