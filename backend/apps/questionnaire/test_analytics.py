...
class QuestionnaireAnalyticsServiceTests(SimpleTestCase):
    ...
    def test_comparison_payload_supports_first_submission_and_specific_cycles(self):
        ...
        abandoned_axis = next(
            item
            for item in payload["lenses"]["adoption"]["axes"]
            if item["label"] == "Abandoned"
        )
        not_adopted_axis = next(
            item
            for item in payload["lenses"]["adoption"]["axes"]
            if item["label"] == "Not adopted"
        )

        self.assertEqual(abandoned_axis["current"], 50)
        self.assertEqual(not_adopted_axis["reference"], 50)
