...
class HistoricalTests(TestCase):
    def setUp(self):
        ...
        self.historical_1 = Historical.objects.create(
            create_at=self.faker.date(), modified_at=self.faker.date()
        )
        self.historical_2 = Historical.objects.create(
            create_at=self.faker.date(), modified_at=self.faker.date()
        )
        self.historical_3 = Historical.objects.create(
            create_at=self.faker.date(), modified_at=self.faker.date()
        )

        self.valid_payload = {
            "create_at": self.faker.date(),
            "modified_at": self.faker.date(),
        }
    ...
    def test_valid_upload(self):
        response = self.client.put(
            reverse("historical-detail", kwargs={"pk": self.historical_1.id}),
            data=json.dumps(self.valid_payload),
            content_type="application/json",
        )
