...
class ExperienceLevelTests(TestCase):
    def setUp(self):
        ...
        self.experiencelevel_1 = ExperienceLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )
        self.experiencelevel_2 = ExperienceLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )
        self.experiencelevel_3 = ExperienceLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )
    ...
    def test_valid_upload(self):
        response = self.client.put(
            reverse("experiencelevel-detail", kwargs={"pk": self.experiencelevel_1.id}),
            data=json.dumps(self.valid_payload),
            content_type="application/json",
        )
    ...
    def test_invalid_get_element(self):
        response = self.client.get(
            reverse("experiencelevel-detail", kwargs={"pk": 666})
        )
