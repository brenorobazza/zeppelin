...
class KnwoledgeLevelTests(TestCase):
    def setUp(self):
        ...
        self.knwoledgelevel_1 = KnwoledgeLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )
        self.knwoledgelevel_2 = KnwoledgeLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )
        self.knwoledgelevel_3 = KnwoledgeLevel.objects.create(
            value=random.uniform(0.00, 100.5)
        )

        self.valid_payload = {"value": random.uniform(0.00, 100.5)}
        self.invalid_payload = {"value": random.uniform(0.00, 100.5)}
    ...
    def test_valid_upload(self):
        response = self.client.put(
            reverse("knwoledgelevel-detail", kwargs={"pk": self.knwoledgelevel_1.id}),
            data=json.dumps(self.valid_payload),
            content_type="application/json",
        )
    ...
    def test_retrieve_all(self):
        ...
        serializer = KnwoledgeLevelSerializer(
            data, context={"request": None}, many=True
        )
    ...
    def test_invalid_delete(self):
        response = self.client.delete(
            reverse("knwoledgelevel-detail", kwargs={"pk": 666})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
