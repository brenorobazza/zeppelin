...
class OrganizationGoogleFormsService(OrganizationAbstractService):
    ...
    def __create_organization(self):
        ...
        organization_state = self.locations[
            self.sheet.cell_value(self.current_line, 4).strip()
        ]

        organization_type_sheet = self.sheet.cell_value(self.current_line, 5).strip()

        orgnization_size = self.organization_sizes[
            self.sheet.cell_value(self.current_line, 6).strip()
        ]
...
class OrganizationService(OrganizationAbstractService):
    ...
    def __create_organization(self):
        ...
        if len(organization) == 0:
            organization = Organization(
                ...
                organizaton_size=organization_size,
                organizaton_type=organization_type,
                age=organization_age,
            )
            organization.save()
        else:
    ...
    def __create_development_team_information(self):
        ...
        development_team_overview = DevelopmentTeamOverview(
            development_team_size=development_team_size,
